...
import { PlusIcon, PencilIcon, TrashIcon, ArrowUpIcon, ArrowDownIcon, UserIcon, ListBulletIcon, Squares2X2Icon } from './ui/Icons.tsx';
...
const PeopleList: React.FC<PeopleListProps> = ({ openPersonForm }) => {
    ...
    const [viewMode, setViewMode] = useState<'list' | 'card'>(
        (localStorage.getItem('peopleListViewMode') as 'list' | 'card') || 'list'
    );

    useEffect(() => {
        localStorage.setItem('peopleListViewMode', viewMode);
    }, [viewMode]);
                ...
                <div className="flex items-center space-x-4">
                    <div className="flex items-center p-1 bg-gray-200 dark:bg-gray-700 rounded-lg">
                        <Tooltip text="List View">
                            <button onClick={() => setViewMode('list')} className={`p-1.5 rounded-md ${viewMode === 'list' ? 'bg-white dark:bg-gray-900 text-blue-600 shadow' : 'text-gray-500 hover:text-gray-600'}`}>
                                <ListBulletIcon />
                            </button>
                        </Tooltip>
                        <Tooltip text="Card View">
                            <button onClick={() => setViewMode('card')} className={`p-1.5 rounded-md ${viewMode === 'card' ? 'bg-white dark:bg-gray-900 text-blue-600 shadow' : 'text-gray-500 hover:text-gray-600'}`}>
                                <Squares2X2Icon />
                            </button>
                        </Tooltip>
                    </div>
                 ...
                 {viewMode === 'list' ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {sortedAndFilteredPeople.map(person => {
                            ...
                            return (
                                <div key={person.id} className={`bg-gray-50 dark:bg-gray-800/50 rounded-lg shadow-md p-4 flex flex-col border-t-4 ${genderClass}`}>
                                    <div className="flex items-start space-x-4 mb-3">
                                        <div className="w-16 h-16 rounded-full flex-shrink-0 flex items-center justify-center bg-gray-200 dark:bg-gray-700 overflow-hidden border-2 border-gray-300">
                                            {person.photos?.[0] ? 
                                                <img src={person.photos[0]} alt={getFullName(person)} className="w-full h-full object-cover" /> : 
                                        ...
                                        <div className="flex-grow">
                                            <div className="flex items-center space-x-2">
                                                {statusDot}
                                                <div 
                                                    className="font-bold text-base cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
                                                    onClick={() => handleEdit(person)}
                                                >
                                                    {getFullName(person)}
                                                </div>
                                    ...
                                    <div className="space-y-1 text-sm flex-grow mb-3">
                                        <div><span className="font-semibold text-gray-600 dark:text-gray-400">Born:</span> {person.birthDate || 'N/A'}</div>
                                        <div><span className="font-semibold text-gray-600 dark:text-gray-400">Died:</span> {person.deathDate || 'N/A'}</div>
                                        <div><span className="font-semibold text-gray-600 dark:text-gray-400">Age:</span> {calculateAge(person.birthDate, person.deathDate)}</div>
                                    </div>
                                    
                                    <div className="mt-auto pt-3 border-t border-gray-200 dark:border-gray-700 flex justify-end">
                                        <div className="flex space-x-2">
                                            <button onClick={() => handleEdit(person)} className="p-1 text-gray-500 hover:text-blue-600 dark:hover:text-blue-400"><PencilIcon /></button>
                                            <button onClick={() => handleDelete(person)} className="p-1 text-gray-500 hover:text-red-600 dark:hover:text-red-400"><TrashIcon /></button>
                                        </div>
